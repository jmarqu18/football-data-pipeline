...
def test_run_feature_engineering_writes_parquet(tmp_path):
    ...
    with engine.begin() as conn:
        conn.execute(
            sa_text("""CREATE TABLE players (
            player_id INTEGER PRIMARY KEY, canonical_name TEXT, known_name TEXT)""")
        )
        conn.execute(
            sa_text("""CREATE TABLE player_season_stats (
        ...
        conn.execute(
            sa_text("""CREATE TABLE player_season_advanced (
        ...
        conn.execute(
            sa_text("""CREATE TABLE player_shots (
            player_id INTEGER, team_id INTEGER, season TEXT,
            x REAL, y REAL, xg REAL, result TEXT, situation TEXT, body_part TEXT)""")
        )
        conn.execute(
            sa_text("""CREATE TABLE player_injuries (
            player_id INTEGER, injury_date TEXT)""")
        )
        conn.execute(
            sa_text("""CREATE TABLE player_transfers (
            player_id INTEGER)""")
        )
